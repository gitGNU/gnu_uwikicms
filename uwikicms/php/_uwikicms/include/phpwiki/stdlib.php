<?php 
   ...
   */
   ...
   function ParseAndLink($bracketlink) {
      ...
      /*
       * Yuck yuck we're using an ugly dirty global hook, but oh well
       * it has a big advantage over passing $page or whatever object
       * as an argument: it keeps intact all the phpwiki function
       * prototypes, and therefore it should be easier to keep up
       * with upstream releases of phpwiki and backport all the
       * good stuff. Plus it's a quick and easy hack 8-)
       */
      global $uwc_page_phpwiki_hook;
	 ...
	 $linktype = 'simple';
      ...
      /*
       * We handle links in a different manner in UWiKiCMS
      ...
      */
      ...
      if (preg_match("#^img:(\\d+)#", $URL, $match)) {
	// Image number $match[1]
	$link['type'] = "uwikicms-img-$linktype";
	$link['link'] = $uwc_page_phpwiki_hook->make_img_link($match[1],$linkname);
      } elseif (preg_match("#^page:((\\d|\\w|\\/|\\-|\\.)+)#", $URL, $match)) {
	// Internal page with relative URL $match[1]
	$link['type'] = "uwikicms-page-$linktype";
	$link['link'] = $uwc_page_phpwiki_hook->make_page_link($match[1],$linkname);
      } elseif (preg_match("#^lang:(\\w{2})#", $URL, $match)) {
	// Internal language change
	$link['type'] = "uwikicms-lang-$linktype";
	$link['link'] = $uwc_page_phpwiki_hook->make_lang_link($match[1],$linkname);
      } else {
