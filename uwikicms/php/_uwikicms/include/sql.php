...
class UWC_Sql {
  var $query=Array(
		   ...
		   "select_content_rss_by_path_and_lang"=>"SELECT SUBSTRING(content_path,%d) AS content_path, content_title, content_text, UNIX_TIMESTAMP(content_date_update) AS content_date_update FROM uwikicms_content WHERE uwikicms_content.content_path LIKE '%s/%%' AND content_lang='%s' AND content_status<=0 AND content_date_update > NOW() - INTERVAL %d day ORDER BY content_date_update DESC LIMIT %d",
