...
class UWC_Sql {
  var $query=Array(
		   ...
		   "select_image_by_container_path"=>"SELECT image_id, image_preview_w, image_preview_h, image_full_w, image_full_h, LENGTH(image_full) AS image_size, image_filename FROM uwikicms_image WHERE image_container_path='%s' ORDER BY image_id",
		   "select_legend_by_container_path_and_lang"=>"SELECT legend_image_id, legend_alt, legend_longdesc FROM uwikicms_legend, uwikicms_image WHERE image_id=legend_image_id AND image_container_path='%s' AND legend_lang='%s' ORDER BY image_id",
